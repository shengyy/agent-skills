...
- **默认单路对抗审查**（一个全面对抗会话 + until-dry 轮次），不要习惯性开三路。
  实践数据：致命发现几乎都出自单路对抗审查的前两轮，多视角并行很少贡献唯一发现，
  三路是纯成本。只有同时满足两条才升级为**按视角拆审**（正确性/回归、数据与状态
  安全、删除完整性各一路）：a) 改动触碰资金/append-only 事实/冻结面/恢复路径等
  不可逆资产；b) diff 大到单会话无法整体装下因果链。升级时在派工说明里写明理由。
